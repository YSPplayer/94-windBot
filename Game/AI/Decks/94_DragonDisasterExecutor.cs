    ...
    public class DragonDisasterExecutor : DefaultExecutor
    {
            ...
            if (Moli_Select_2 || Moli_Select_3)
            {
                ...
                for (int i = 0; i < deck_cards.Count(); ++i)
                {
                    if ((Bot.Hand.Count > 0 && deck_cards[i].Id == MonsterId.Y_TheWatery)
                          || deck_cards[i].Id == MonsterId.Y_TheMirror || (Bot.GetCountCardInZone(Bot.Banished, 7, 0, true, false) > 0
        ...
        private bool DasuoEffect()
        {
            if (Bot.LifePoints > 1000)
            {
                ...
                else if (Bot.GetCountCardInZone(Bot.Banished, 10, CardRace.Fiend) > 0)
                {
        ...
        {
            if (Bot.GetMonsterCount() == 1 && Bot.GetCountCardInZone(Bot.GetMonsters(), CardType.Monster, 0x7cca, true, false) < 1) return GetBeastAttack();
            ...
            if (Duel.Player == 0 && Bot.GetMonsterCount() <= 2 && Bot.GetCountCardInZone(Bot.GetMonsters(), CardType.Monster, 0x7cca, true, false) < 2) return GetBeastAttack();
                ...
                switch (Card.Id)
                {
                    ...
                    case MonsterId.Z_luoji:
                        ...
                        if (card != null && card.HasType(CardType.Monster) && card.Controller == 1 && !card.IsShouldNotBeMonsterTarget() && card.Location == CardLocation.MonsterZone)
                        {
                            ...
                            List<ClientCard> handCards = Bot.GetCardInZone(Bot.Hand, CardType.Monster, 0x7cca);
                            List<ClientCard> monsterCards = Bot.GetCardInZone(Bot.MonsterZone, CardType.Monster, 0x7cca);
                    ...
                    case MonsterId.Z_aoding:
                        ...
                        if (Duel.Phase == DuelPhase.Main1 || Duel.Phase == DuelPhase.Main2)
                        {
                            ...
                            List<ClientCard> pre_costCards = Bot.GetCardInZone(cards, 0x7cca, true, false);
        ...
        private bool TaidongActive()
        {
            return ((Card.Location == CardLocation.Hand || (Card.Location == CardLocation.Onfield
                   ...
                   && (Bot.GetCountCardInZone(Bot.GetHands(), CardType.Monster, 0x7cca, true, false) > 0 || Bot.GetCountCardInZone(Bot.Banished, CardType.Monster, 0x7cca, true, false) > 0));
        }
        ...
        private bool TaidongEffect()
        {
            if (Card.IsFaceup() && Card.Location != CardLocation.Hand)
            {
                ...
                if ((Bot.GetCountCardInZone(Bot.Banished, CardType.Monster, 0x7cca, true, false) +
                    Bot.GetCountCardInZone(Bot.Hand, CardType.Monster, 0x7cca, true, false)) > 2)
                {
                    List<ClientCard> reCards = Bot.GetCardInZone(Bot.Banished, CardType.Monster, 0x7cca, true, false);
